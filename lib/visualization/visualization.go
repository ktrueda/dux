...
func GraphString(data map[string]int64) string {
	...
	for _, d := range sorter {
		line := graphLine(d.Key, d.Value, normalizedData[d.Key], "B", labelFormat)
...
func graphLine(label string, val int64, length int, unit string, labelFomat string) string {
	var buffer bytes.Buffer
